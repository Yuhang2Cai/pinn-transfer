...
plt.rcParams['figure.figsize'] = (12, 6)  # 确保宽和高都是正值
plt.rcParams['figure.dpi'] = 300  # 设置DPI以保证清晰度
...
class My_loss(nn.Module):
    ...
    def forward(self, targets_P, dpdt, mdot_A, V, outputs_P, bulk_modulus_model, air_dissolution_model,
        ...
        for i in range(num):
            ...
            # print(f"dpdt:{dpdt[i][0]} ")
            # print(f"mdot_A:{mdot_A[i]}")
            # print(f"p:{outputs_P[i]}")
            # 计算测量值损失（使用MSE）
...
class TriplexPINN(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, layers, scaler_inputs, scaler_targets):
        ...
        self.time_scale = 10000.0  # 时间缩放因子
    ...
    def forward(self, inputs):
        ...
        P_t = torch.autograd.grad(
            ...
            retain_graph=True,
            only_inputs=True
        )[0] * (1.0 / self.time_scale)  # 恢复原始时间尺度
...
def main(weight):
    ...
    _, mean_inputs_train, std_inputs_train = standardize_tensor(torch.reshape(inputs_train, (num, 1, 3)), mode='fit')
    ...
    batch_size = BATCH_SIZE
    epochs = NUM_EPOCH
    time_scale = model.time_scale if hasattr(model, 'time_scale') else 1.0  # 时间缩放倍数
    physics_weight = criterion.physics_weight if hasattr(criterion, 'physics_weight') else 1.0  # 物理权重
    num_layers = len(model.surrogateNN.layers) // 3  # 计算神经网络层数
    neurons_per_layer = NUM_NEURONS  # 每层神经元个数
    ...
    # Calculate complete metrics for each dataset
    # Training set
    ...
    metrics_df = pd.DataFrame({
        ...
        "RMSE": [metrics_data["train_set"]["RMSE"], metrics_data["val_set"]["RMSE"], metrics_data["test_set"]["RMSE"]],
        "MAE": [metrics_data["train_set"]["MAE"], metrics_data["val_set"]["MAE"], metrics_data["test_set"]["MAE"]],
        "MAPE": [metrics_data["train_set"]["MAPE"], metrics_data["val_set"]["MAPE"], metrics_data["test_set"]["MAPE"]],
        "R2": [metrics_data["train_set"]["R2"], metrics_data["val_set"]["R2"], metrics_data["test_set"]["R2"]]
    })
